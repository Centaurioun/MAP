...
Public ExternalColumnSorter As IExtColSort

#If 0 Then
    Dim X, y, Column, nextone 'force lowercase so ide doesnt switch around on its own whim...
#End If

Sub ChangeColHeaderText(index As Long, newText As String)
    On Error Resume Next
    lv.ColumnHeaders(index).text = newText
    lvFilter.ColumnHeaders(index).text = newText
End Sub
...
Public Sub ColumnSort(Column As ColumnHeader)
    ...
    If Not ExternalColumnSorter Is Nothing Then
        ExternalColumnSorter.ExternalColumnSort ListViewControl, Column
        Exit Sub
    End If
