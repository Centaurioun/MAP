...
Event ItemDeleted(Item As MSComctlLib.ListItem, ByRef cancel As Boolean) 'handle each item (like delete file or entry from db)
...
Property Get SubItem(liOrIndex As Variant, colName As String) As String
       
       Dim i As Long
       Dim li As ListItem
       
       On Error Resume Next
       If IsObject(liOrIndex) Then
            Set li = liOrIndex
            If Err.Number <> 0 Then Exit Property
       Else
            Set li = lv.ListItems(liOrIndex)
            If Err.Number <> 0 Then Exit Property
       End If
       
       'find the index from column name
       For i = 1 To lv.ColumnHeaders.count
            If lv.ColumnHeaders(i).Text = colName Then Exit For
       Next
                
       If i = lv.ColumnHeaders.count + 1 Then Exit Property 'not found
        
       If i = 1 Then
            SubItem = li.Text
       Else
            SubItem = li.subItems(i - 1)
       End If
    
End Property

Property Let SubItem(liOrIndex As Variant, colName As String, newVal As String)
...
Property Get selItem() As ListItem
    ...
    'there is no user selection, but only one filtered list entry
    'this is as good as a user selection..
    If lvFilter.Visible Then
        If selItem Is Nothing And currentLV.ListItems.count = 1 Then
            Set selItem = currentLV.ListItems(1)
        End If
    End If
...
Function AddItem(txt, ParamArray subItems()) As ListItem
    ...
    Dim i As Integer, lis As ListSubItem
    ...
    For Each si In subItems
        ...
        Set lis = AddItem.ListSubItems.Add(, , si)
        'lis.key = lv.ColumnHeaders(i + 2).key 'may throw an error so on its own line..
        i = i + 1
    Next
...
Sub SetColumnHeaders(csvList As String, Optional csvWidths As String)
    ...
    On Error Resume Next
    Dim i As Long, fc As Long, ch As ColumnHeader, ch2 As ColumnHeader
    ...
    For Each t In tmp
        ...
        End If
        Set ch = lv.ColumnHeaders.Add(, , Trim(t))
        Set ch2 = lvFilter.ColumnHeaders.Add(, , Trim(t))
        
        'can throw an error if not unique so not on same line
        'ch.key = ch.Text
        'ch2.key = ch.Text
    Next
...
Private Sub lv_KeyDown(keyCode As Integer, shift As Integer)
    ...
    If keyCode = vbKeyDelete And AllowDelete Then
        ...
        If cnt > 0 Then RaiseEvent DeletesComplete(cnt)
...
Private Sub txtFilter_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = vbKeyReturn Then
        ...
        If Len(txtFilter) > 0 Then
            ...
            End If
        End If
        RaiseEvent UserHitReturnInFilter
    End If
